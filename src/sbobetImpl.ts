...
import {Grabber, Options, Sport} from '../jmonitor-interface';
...
export class  SbobetImpl{
	...
	constructor(){};
	setDynamicjs = (htmlString:string):string =>{
	...
	setToken = (htmlString:string):string =>{
	...
	handlerProgram = (err, data) =>{
		...
		if (err) {
			...
			mylog.debug("taken dinamicjs:" + dinamicjs +" token:"+ token);

			var uri:string = "http://www.sbobet.com/en/resource/e/" + this.dynamicjs;
			 grabber.httpCall(
				new Options(uri), 
				(err, htmlString)=>{
					if (err) {
